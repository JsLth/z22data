...
walk(dbListTables(con), .progress = "Exporting tables", function(table) {
  dbExecute(con, sprintf(
    "COPY %s TO 'z11_data_100m/%s.parquet' (FORMAT PARQUET, CODEC 'zstd', COMPRESSION_LEVEL -7);",
    table, table
  ))
  cat(create_schema(con, table), "\n", file = "z11_data_100m/schema.sql", append = TRUE)
  cat(create_load(con, table), "\n", file = "z11_data_100m/load.sql", append = TRUE)
})
